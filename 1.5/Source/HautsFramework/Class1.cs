...
namespace HautsFramework
{
    ...
    public class PermitMoreEffects : DefModExtension
    {
        ...
        //other numerical effect
        public FloatRange extraNumber;
        //thing-targeting e.g. investments
    ...
    public class RoyalTitlePermitWorker_DropBook : RoyalTitlePermitWorker_Targeted
    {
            ...
            foreach (BookOutcomeDoer bookOutcomeDoer in book.BookComp.Doers)
            {
                ...
                if (topicRulePacks != null)
                {
                    foreach (RulePack rulePack in topicRulePacks)
                    {
                        ...
                        RoyalTitlePermitWorker_DropBook.subjects.Add(new RoyalTitlePermitWorker_DropBook.BookSubjectSymbol
                        {
        ...
        private static void AppendRulesForSubject(List<RoyalTitlePermitWorker_DropBook.BookSubjectSymbol> subjects, List<Verse.Grammar.Rule> rules, Dictionary<string, string> constants, string postfix, int i)
        {
            if (i < subjects.Count)
            {
        ...
        private static readonly Texture2D CommandTex = ContentFinder<Texture2D>.Get("UI/Commands/CallAid", true);
        private static List<RoyalTitlePermitWorker_DropBook.BookSubjectSymbol> subjects = new List<RoyalTitlePermitWorker_DropBook.BookSubjectSymbol>();
    ...
    public class RoyalTitlePermitWorker_DropResourcesOfCategory : RoyalTitlePermitWorker_Targeted
    {
        ...
        private void CallResourcesToCaravan(Pawn caller, Faction faction, bool free)
        {
            ...
            if (pme != null && pme.thingCategory != null)
            {
                ...
                for (int i = 0; i < pme.numFromCategory.RandomInRange; i++)
                {
                    DefDatabase<ThingDef>.AllDefsListForReading.TryRandomElement((ThingDef x) => x.thingCategories.Contains(pme.thingCategory) && x.techLevel <= pme.maxTechLevelInCategory && x.techLevel >= pme.minTechLevelInCategory && x.BaseMarketValue <= pme.marketValueLimits.max && x.BaseMarketValue >= pme.marketValueLimits.min, out ThingDef randomThing);
                    if (randomThing != null)
                    {
                        Thing thing = ThingMaker.MakeThing(randomThing, GenStuff.RandomStuffFor(randomThing));
                        if (thing.TryGetComp(out CompQuality compQuality))
    ...
    public class RoyalTitlePermitWorker_GenerateQuest : RoyalTitlePermitWorker_Targeted
    {
        ...
        private void GiveQuest(Pawn caller, Faction faction, IncidentParms parms, bool free)
        {
            PermitMoreEffects pme = this.def.GetModExtension<PermitMoreEffects>();
